...
---
= Configurare Rocky Linux 8.9 per NVMe-oF con storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Scopri come configurare gli host NVMe over Fabrics (NVMe-oF) per Rocky Linux 8.9. Per ulteriori informazioni sul supporto e sulle funzionalità, vedere link:nvme-rockylinux-supported-features.html["Supporto e funzionalità di Rocky Linux ONTAP"].

*NVMe-oF con Rocky Linux 8.9 presenta le seguenti limitazioni note:*

* L'avvio SAN tramite il protocollo NVMe-oF non è attualmente supportato.
* Il multipath NVMe nel kernel è disabilitato per impostazione predefinita sugli host NVMe-oF in Rocky Linux 8.9; è necessario abilitarlo manualmente.
...
.Fasi
. Installare Rocky Linux 8.9 sul server.  Una volta completata l'installazione, verifica di avere installato il kernel Rocky Linux 8.9 richiesto:
+
[source, cli]
----
uname -r
