...
* A partire da ONTAP 9.16.1, l'allocazione dello spazio è abilitata per impostazione predefinita per tutti i namespace NVMe appena creati.
* A partire da ONTAP 9.9.1 P3, il protocollo NVMe/FC è supportato per ESXi 8.
...
| link:https://mysupport.netapp.com/site/bugs-online/product/ONTAP/BURT/1420654["1420654"^] | Il nodo ONTAP non è operativo quando si utilizza il protocollo NVMe/FC con la versione di ONTAP 9.9.1 | ONTAP 9.9.1 ha introdotto il supporto per il comando "abort" di NVMe. Quando ONTAP riceve il comando "abort" per interrompere un comando NVMe fuso in attesa del comando partner, si verifica un'interruzione del nodo ONTAP. Il problema si verifica solo con gli host che utilizzano comandi NVMe fusi (ad esempio, ESX) e il trasporto Fibre Channel (FC). 


| link:https://jira.ngage.netapp.com/browse/CONTAPEXT-1126["CONTAPEXT-1126"^] | L'errore i/o si verifica quando le macchine virtuali Linux che utilizzano gli adattatori vNVMe incontrano una lunga finestra APD (All Paths Down)  a| 
Le macchine virtuali Linux che eseguono vSphere 8.x e versioni successive e utilizzano adattatori NVMe virtuali (vNVMe) riscontrano un errore di I/O perché l'operazione di ritentativo vNVMe è disabilitata per impostazione predefinita. Per evitare un'interruzione sulle macchine virtuali Linux che eseguono kernel meno recenti durante un all paths down (APD) o un carico di I/O elevato, VMware ha introdotto un parametro regolabile "VSCSIDisableNvmeRetry" per disabilitare l'operazione di ritentativo vNVMe.
