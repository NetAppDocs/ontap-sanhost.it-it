...
summary: Descrive come utilizzare Oracle Linux 8.3 con ONTAP 
---
= Utilizza Oracle Linux 8.3 con ONTAP
:hardbreaks:
:toclevels: 1P
:allow-uri-read: 
:toclevels: 1P
...
[role="lead"]
È possibile utilizzare le impostazioni di configurazione dell'host SAN di ONTAP per configurare Oracle Linux 8,3 con ONTAP come destinazione.
...
Per Oracle Linux (OL) 8.3 il file /etc/multipath.conf deve esistere, ma non è necessario apportare modifiche specifiche al file. OL 8.3 viene compilato con tutte le impostazioni necessarie per riconoscere e gestire correttamente le LUN ONTAP.
...
Il sistema operativo Oracle Linux 8.3 viene compilato per riconoscere le LUN ONTAP e impostare automaticamente tutti i parametri di configurazione correttamente per la configurazione ASA e non ASA.
...
NOTE: Per configurare Oracle Linux 8.3 Red Hat Enterprise kernel (RHCK), utilizzare link:hu_rhel_83.html#recommended-settings["impostazioni consigliate"] Per Red Hat Enterprise Linux (RHEL) 8.3.



== Problemi noti

Non ci sono problemi noti per Oracle Linux 8,3 con ONTAP release.


NOTE: Per i problemi noti di Oracle Linux (kernel compatibile con Red Hat), consultare link:hu_rhel_83.html#known-problems-and-limitations["problemi noti"] Nella corrispondente documentazione sulla release di Red Hat Enterprise Linux.
