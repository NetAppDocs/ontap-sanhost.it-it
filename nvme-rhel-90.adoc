...
---
= Configurare RHEL 9.0 per NVMe-oF con storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Scopri come configurare gli host NVMe over Fabrics (NVMe-oF) per RHEL 9.0.  Per ulteriori informazioni sul supporto e sulle funzionalità, vederelink:hu-nvme-index.html["Panoramica NVME-oF"^] .

*NVMe-oF con RHEL 9.0 presenta le seguenti limitazioni note:*
...
.Fasi
. Installare RHEL 9.0 sul server.  Una volta completata l'installazione, verificare di avere in esecuzione il kernel RHEL 9.0 richiesto:
+
[source, cli]
----
uname -r
...
----
. Sull'host RHEL 9.0, controllare `hostnqn` stringa a `/etc/nvme/hostnqn` :
+
[source, cli]
----
