...
--
.Fasi
. Il driver inbox qla2xxx nativo incluso nel kernel Ubuntu 22,04 GA ha le ultime correzioni upstream essenziali per il supporto di ONTAP. Verificare che siano in esecuzione le versioni del firmware e del driver dell'adattatore supportate:
+
[listing]
----
...
Theere non sono problemi noti per la configurazione host NVMe-of per Ubuntu 22,04 con ONTAP release.
