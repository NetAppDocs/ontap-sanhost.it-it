...
summary: 'Come configurare l"host NVMe-oF per RHEL 8.x con ONTAP' 
---
= Configurare RHEL 8.x per NVMe-oF con storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Scopri come configurare gli host NVMe over Fabrics (NVMe-oF) per RHEL 8.x. Per ulteriori informazioni sul supporto e sulle funzionalità, vedere link:nvme-rhel-supported-features.html["Supporto e funzionalità RHEL ONTAP"].

NVMe-oF con RHEL 8.x presenta le seguenti limitazioni note:

* L'avvio SAN tramite il protocollo NVMe-oF non è attualmente supportato.
* Il multipath NVMe nel kernel è disabilitato per impostazione predefinita sugli host NVMe-oF in RHEL 8.x; è necessario abilitarlo manualmente.
...
.Fasi
. Installare RHEL 8.x sul server. Una volta completata l'installazione, verificare di avere in esecuzione il kernel RHEL 8.x richiesto:
+
[source, cli]
----
uname -r
...
----
. Sull'host RHEL 8.x, controllare `hostnqn` stringa a `/etc/nvme/hostnqn` :
+
[source, cli]
----
