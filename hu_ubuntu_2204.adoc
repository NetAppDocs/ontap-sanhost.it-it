...
summary: Installa Linux Host Utilities su un host Ubuntu 22.04 per gestire le operazioni dei protocolli FCP e iSCSI con le LUN ONTAP. 
---
= Configurare Ubuntu 22.04 per FCP e iSCSI con storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Il software Linux Host Utilities fornisce strumenti di gestione e diagnostica per gli host Linux connessi allo storage ONTAP .  Quando si installa Linux Host Utilities su un host Ubuntu 22.04, è possibile utilizzare Host Utilities per gestire le operazioni del protocollo FCP e iSCSI con LUN ONTAP .
...
Con Ubuntu 22.04 è possibile utilizzare il multipathing per gestire le LUN ONTAP .
