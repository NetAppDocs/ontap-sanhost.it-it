...
NVMe/FC è supportato su ONTAP 9.6 o versioni successive per le seguenti versioni di SUSE Linux Enterprise Server:

* SUSE Linux Enterprise Server 15 SP1
+
L'host SUSE Linux Enterprise Server 15 SP1 può eseguire sia il traffico NVMe/FC che FCP tramite le stesse porte dell'adattatore iniziatore Fibre Channel. Consultare la link:https://hwu.netapp.com/Home/Index["Hardware Universe"^] per un elenco di controller e adattatori FC supportati.
...
* Gli script nativi di connessione automatica NVMe/FC sono inclusi nel pacchetto nvme-cli. È possibile utilizzare il driver lpfc inbox nativo su SUSE Linux Enterprise Server 15 SP1.
...
== Abilitare NVMe/FC su SUSE Linux Enterprise Server 15 SP1

. Eseguire l'aggiornamento al kernel MU di SUSE Linux Enterprise Server 15 SP2 consigliato
. Eseguire l'aggiornamento alla versione consigliata di nvme-cli MU.
+
Questo pacchetto nvme-cli contiene gli script di connessione automatica NVMe/FC nativi, quindi non è necessario installare gli script di connessione automatica NVMe/FC esterni forniti da Broadcom sull'host SUSE Linux Enterprise Server 15 SP1. Questo pacchetto include anche la regola udev di ONTAP che consente il bilanciamento del carico round-robin per il multipath NVMe e il plug-in NetApp per i dispositivi ONTAP.
...
----
. Sull'host SUSE Linux Enterprise Server 15 SP1, controllare la stringa NQN host su `/etc/nvme/hostnqn` e verificare che corrisponda alla stringa NQN host per il sottosistema corrispondente sull'array ONTAP. Ad esempio:
+
[listing]
----
