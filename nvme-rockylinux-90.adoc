...
---
= Configurare Rocky Linux 9.0 per NVMe-oF con storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Scopri come configurare gli host NVMe over Fabrics (NVMe-oF) per Rocky Linux 9.0. Per ulteriori informazioni sul supporto e sulle funzionalità, vedere link:nvme-rockylinux-supported-features.html["Supporto e funzionalità di Rocky Linux ONTAP"].

*NVMe-oF con Rocky Linux 9.0 presenta le seguenti limitazioni note:*
...
.Fasi
. Installare Rocky Linux 9.0 sul server.  Una volta completata l'installazione, verifica di avere installato il kernel Rocky Linux 9.0 richiesto:
+
[source, cli]
----
uname -r
