...
summary: Installa Linux Host Utilities su un host CentOS 8.x per gestire le operazioni del protocollo FCP e iSCSI con LUN ONTAP . 
---
= Configurare CentOS 8.x per FCP e iSCSI per l'archiviazione ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Il software Linux Host Utilities fornisce strumenti di gestione e diagnostica per gli host Linux connessi allo storage ONTAP . Quando si installa Linux Host Utilities su un host CentOS 8.x, è possibile utilizzare Host Utilities per gestire le operazioni del protocollo FCP e iSCSI con LUN ONTAP .
...
È possibile utilizzare il multipathing con CentOS 8.x per gestire le LUN ONTAP .
...
Per i problemi noti del kernel compatibile con CentOS Red Hat, vedere la link:hu-rhel-8x.html#step-7-review-the-known-issues["problemi noti"] per la serie Red Hat Enterprise Linux (RHEL) 8.x.
