...
summary: Descrive come utilizzare HP-UX Host Utilities 6.0 con ONTAP 
---
= Utilizzare i comandi di HP-UX Host Utilities per verificare la configurazione dell'archiviazione ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Utilizzare il riferimento ai comandi di esempio di HP-UX Host Utilities 6.0 per una convalida end-to-end della configurazione di archiviazione ONTAP mediante lo strumento Host Utilities.



== Elenca tutti gli iniziatori host mappati all'host

Recupera un elenco di tutti gli iniziatori host mappati su un host.
...
Recupera un elenco di tutti i LUN mappati su un host.
...
Recupera un elenco di tutti i LUN mappati su un host da una SVM specifica.
...
Recupera un elenco di tutti gli attributi di una LUN specificata mappata su un host.
...
Recupera un elenco di attributi LUN ONTAP in base al nome file del dispositivo host specificato.
...
Recupera un elenco di tutti i WWPN LIF di destinazione SVM collegati a un host.
