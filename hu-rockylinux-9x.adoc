...
summary: Installare Linux Host Utilities su un host Rocky Linux 9.x per gestire le operazioni del protocollo FCP e iSCSI con LUN ONTAP . 
---
= Configurare Rocky Linux 9.x per FCP e iSCSI con storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Il software Linux Host Utilities fornisce strumenti di gestione e diagnostica per gli host Linux connessi allo storage ONTAP . Quando si installa Linux Host Utilities su un host Rocky Linux 9.x, è possibile utilizzare Host Utilities per gestire le operazioni del protocollo FCP e iSCSI con LUN ONTAP .
...
Con Rocky Linux 9.x è possibile utilizzare il multipathing per gestire le LUN ONTAP .
...
Rocky Linux 9.x con storage ONTAP presenta i seguenti problemi noti.
...
| link:https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/1508554["1508554"^] | La CLI delle NetApp Linux host Utilities richiede dipendenze aggiuntive dei pacchetti di librerie per supportare il rilevamento dell'adattatore HBA Emulex | In Rocky Linux 9.1, la CLI NetApp Linux SAN Host Utilities  `sanlun fcp show adapter -v` fallisce perché non è possibile trovare le dipendenze del pacchetto libreria per supportare la scoperta HBA. 
|===
--
====
