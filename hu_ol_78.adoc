...
summary: Installare Linux Host Utilities su un host Oracle Linux 7.8 per gestire le operazioni del protocollo FCP e iSCSI con LUN ONTAP. 
---
= Configurare Oracle Linux 7.8 per FCP e iSCSI con storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Il software Linux host Utilities fornisce strumenti di gestione e diagnostica per gli host Linux connessi allo storage ONTAP. Quando si installa Linux Host Utilities su un host Oracle Linux 7.8, è possibile utilizzare Host Utilities per gestire le operazioni del protocollo FCP e iSCSI con LUN ONTAP.
...
Puoi utilizzare multipathing con Oracle Linux 7,8 per gestire i LUN di ONTAP.


NOTE: Puoi usare il link:hu_rhel_78.html#rhel-rhck["impostazioni consigliate per Red Hat Enterprise Linux (RHEL) 7.8"] per configurare il kernel compatibile con Red Hat per Oracle Linux 7.8.
...
L'host Oracle Linux 7.8 con storage ONTAP presenta i seguenti problemi noti:
