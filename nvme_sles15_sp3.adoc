...
[role="lead"]
NVMe over Fabrics o NVMe-of (inclusi NVMe/FC e altri trasporti) è supportato da SUSE Linux Enterprise Server 15 SP3 con ANA (Asymmetric Namespace Access). ANA è l'equivalente ALUA nell'ambiente NVMe-of ed è attualmente implementato con il multipath NVMe nel kernel. Utilizzando questa procedura, è possibile abilitare NVMe-of con multipath NVMe in-kernel utilizzando ANA su SUSE Linux Enterprise Server 15 SP3 e ONTAP come destinazione.
...
* Non esiste alcun supporto sanlun per NVMe-of. Pertanto, non è disponibile il supporto LUHU per NVMe-of su SUSE Linux Enterprise Server 15 SP3. Puoi utilizzare il plug-in NetApp incluso nel pacchetto nvme-cli nativo per NVMe-of. Questo dovrebbe supportare tutti i trasporti NVMe-of.
