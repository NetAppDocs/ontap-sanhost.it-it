...
summary: 'Come configurare l"host NVMe-oF per Rocky Linux 8.7 con ONTAP' 
---
= Configurare Rocky Linux 8.7 con NVMe-oF per l'archiviazione ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Gli host Rocky Linux 8.7 supportano i protocolli NVMe/FC e NVMe/TCP con Asymmetric Namespace Access (ANA). ANA è equivalente al multipathing ALUA (Asymmetric Logical Unit Access) negli ambienti iSCSI e FCP ed è implementato utilizzando la funzionalità multipath NVMe integrata nel kernel.
...
È possibile utilizzare il supporto e le funzionalità seguenti con la configurazione host NVMe-oF per Rocky Linux 8.7. Prima di avviare il processo di configurazione, è inoltre necessario esaminare le limitazioni note.
...
* Limitazioni note:
+
** Per impostazione predefinita, il multipath NVMe nel kernel è disabilitato per gli host Rocky Linux 8.7 NVMe-oF. Pertanto, è necessario attivarlo manualmente.
** Sugli host Rocky Linux 8.7, NVMe/TCP è una funzionalità di anteprima tecnologica a causa di problemi aperti. Fare riferimento a link:https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/8/html-single/8.7_release_notes/index#technology-preview_file-systems-and-storage["Note di rilascio di Rocky Linux 8.7"^] per maggiori dettagli.
...
Utilizzare la seguente procedura per convalidare le versioni minime supportate del software Rocky Linux 8.7.

.Fasi
. Installa Rocky Linux 8.7 sul server. Al termine dell'installazione, verifica di utilizzare il kernel Rocky Linux 8.7 specificato:
+
[source, cli]
----
uname -r
...
La configurazione host NVMe-oF per Rocky Linux 8.7 con storage ONTAP presenta i seguenti problemi noti:
