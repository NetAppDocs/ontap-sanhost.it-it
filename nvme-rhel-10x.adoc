...
summary: 'Come configurare l"host NVMe-oF per RHEL 10x con ONTAP' 
---
= Configurare RHEL 10x per NVMe-oF con storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Scopri come configurare gli host NVMe over Fabrics (NVMe-oF) per RHEL 10x. Per ulteriori informazioni sul supporto e sulle funzionalità, vedere link:nvme-rhel-supported-features.html["Supporto e funzionalità RHEL ONTAP"].

NVMe-oF con RHEL 10x presenta le seguenti limitazioni note:
...
.Fasi
. Installare RHEL 10x sul server.  Una volta completata l'installazione, verificare di avere in esecuzione il kernel RHEL 10x richiesto:
+
[source, cli]
----
uname -r
...
L'autenticazione in-band sicura è supportata tramite NVMe/TCP tra un host RHEL 10x e un controller ONTAP .
...
cat /etc/nvme/hostnqn
----
. Genera la chiave dhchap per l'host RHEL 10x.
+
