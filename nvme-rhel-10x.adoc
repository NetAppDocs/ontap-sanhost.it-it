...
.Modifiche a iopolicy per le versioni di RHEL 10.x
* A partire da RHEL 10.1, la policy di I/O predefinita è queue-depth.
* In RHEL 10.0, round-robin è la iopolicy predefinita, ma queue-depth è disponibile come opzione configurabile.


Se si utilizza RHEL 10.0 e si desidera modificare l'iopolicy in queue-depth, modificare il file delle regole udev come segue:
...
.Fasi
. Verificare che le impostazioni NVMe-oF appropriate (ad esempio, modello impostato su NetApp ONTAP Controller e iopolicy di bilanciamento del carico impostato su queue-depth) per i rispettivi namespace ONTAP siano visualizzate correttamente sull'host:
+
