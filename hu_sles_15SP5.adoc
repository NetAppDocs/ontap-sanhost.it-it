...
summary: Viene descritto come utilizzare SUSE Linux Enterprise Server 15 SP5 con ONTAP 
---
= Utilizzate SUSE Linux Enterprise Server 15 SP5 con ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
È possibile utilizzare le impostazioni di configurazione dell'host SAN di ONTAP per configurare SUSE Linux Enterprise Server 15 SP5 con ONTAP come destinazione.
...
Per SUSE Linux Enterprise Server 15 SP5, la `/etc/multipath.conf` il file deve esistere, ma non è necessario apportare modifiche specifiche al file. SUSE Linux Enterprise Server 15 SP5 è compilato con tutte le impostazioni necessarie per riconoscere e gestire correttamente i LUN ONTAP.
