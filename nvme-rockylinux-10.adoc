...
summary: 'Come configurare l"host NVMe-oF per Rocky Linux 10.0 con storage ONTAP' 
---
...
Scopri come configurare gli host NVMe over Fabrics (NVMe-oF) per Rocky Linux 10.0. Per ulteriori informazioni sul supporto e sulle funzionalità, vedere link:nvme-rockylinux-supported-features.html["Supporto e funzionalità di Rocky Linux ONTAP"].
...
.Fasi
. Installare Rocky Linux 10.0 sul server.  Una volta completata l'installazione, verifica di avere installato il kernel Rocky Linux 10.0 richiesto:
+
[source, cli]
----
uname -r
...
cat /etc/nvme/hostnqn
----
. Genera la chiave dhchap per l'host Rocky Linux 10.0.
+
