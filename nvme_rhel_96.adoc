...
----
. Sull'host RHEL, controlla la stringa hostnqn in  `/etc/nvme/hostnqn` :
+
[source, cli]
----
...
Con RHEL 9,6, il  `nvmefc-boot-connections.service` E  `nvmf-autoconnect.service` servizi di avvio inclusi in NVMe/FC  `nvme-cli` I pacchetti vengono abilitati automaticamente all'avvio del sistema. Al termine dell'avvio, verificare che i servizi siano abilitati.
