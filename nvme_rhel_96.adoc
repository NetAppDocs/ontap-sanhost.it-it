...
summary: Come configurare NVMe-of host per RHEL 9,6 con ONTAP 
---
= Configurazione host NVMe-of per RHEL 9,6 con ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Con la configurazione host NVMe-of per Red Hat Enterprise Linux (RHEL) 9,6 puoi utilizzare il seguente supporto e funzionalità. Prima di avviare il processo di configurazione, è inoltre necessario esaminare le limitazioni note.
...
** RHEL 9,6 abilita la multipath NVMe in-kernel per namespace NVMe per default, eliminando la necessità di impostazioni esplicite.
** A partire da ONTAP 9.12.1, viene introdotto il supporto per l'autenticazione sicura in-band per NVMe/TCP. È possibile utilizzare l'autenticazione sicura in-band per NVMe/TCP con RHEL 9,6.
+
...
Utilizzare la seguente procedura per convalidare le versioni minime supportate del software RHEL 9,6.

.Fasi
. Installare RHEL 9,6 sul server. Una volta completata l'installazione, verificare che il kernel RHEL 9,6 specificato sia in esecuzione:
+
[source, cli]
----
uname -r
...
----
. Sull'host RHEL 9,6, controllare la stringa hostnqn all'indirizzo `/etc/nvme/hostnqn`:
+
[source, cli]
----
...
Con RHEL 9.6, il  `nvmefc-boot-connections.service` E  `nvmf-autoconnect.service` servizi di avvio inclusi in NVMe/FC  `nvme-cli` I pacchetti vengono automaticamente abilitati per l'avvio durante l'avvio del sistema. Al termine dell'avvio del sistema, verificare che i servizi di avvio siano abilitati.
...
A partire da ONTAP 9.12.1, l'autenticazione in-band sicura è supportata tramite NVMe/TCP tra un host RHEL 9,6 e un controller ONTAP.
