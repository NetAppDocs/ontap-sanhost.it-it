...
summary: Installare le Linux Host Utilities su un host Oracle Linux 10.x per gestire le operazioni dei protocolli FCP e iSCSI con LUN ONTAP. 
---
= Configurare Oracle Linux 10.x per FCP e iSCSI con storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Il software Linux Host Utilities fornisce strumenti di gestione e diagnostica per gli host Linux connessi allo storage ONTAP. Quando si installa Linux Host Utilities su un host Oracle Linux 10.x, è possibile utilizzare Host Utilities per gestire le operazioni dei protocolli FCP e iSCSI con le LUN ONTAP.
...
È possibile utilizzare il multipathing con Oracle Linux 10.x per gestire le LUN ONTAP.
