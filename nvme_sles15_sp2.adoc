...
[role="lead"]
NVMe/FC è supportato su ONTAP 9.6 e versioni successive con SUSE Linux Enterprise Server 15 SP2. L'host SUSE Linux Enterprise Server 15 SP2 può eseguire sia il traffico NVMe/FC, sia il traffico FCP tramite le stesse porte dell'adattatore iniziatore Fibre Channel. Consultare la link:https://hwu.netapp.com/Home/Index["Hardware Universe"^] per un elenco di controller e adattatori FC supportati.
