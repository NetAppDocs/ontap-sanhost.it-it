...
summary: Descrive come utilizzare Oracle Linux 8.x con ONTAP 
---
= Configurare Oracle Linux 8.x per FCP e iSCSI con storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Il software Linux Host Utilities fornisce strumenti di gestione e diagnostica per gli host Linux connessi allo storage ONTAP . Quando si installa Linux Host Utilities su un host Oracle Linux 8.x, è possibile utilizzare Host Utilities per gestire le operazioni del protocollo FCP e iSCSI con LUN ONTAP .
...
È possibile utilizzare il multipathing con Oracle Linux 8.x per gestire le LUN ONTAP .


NOTE: Puoi usare il link:hu-rhel-8x.html#rhel-rhck["impostazioni consigliate per Red Hat Enterprise Linux (RHEL) 8.x"] per configurare il kernel compatibile con Red Hat per Oracle Linux 8.x.
...
== Passaggio 4: conferma la configurazione iSCSI per l'host
...
== Passaggio 5: facoltativamente, escludi un dispositivo dal multipathing
...
== Passaggio 6: personalizzare i parametri multipath per le LUN ONTAP
...
* Scopri di più sul mirroring ASM
+
