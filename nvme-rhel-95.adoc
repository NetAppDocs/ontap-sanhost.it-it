...
Scopri come configurare gli host NVMe over Fabrics (NVMe-oF) per Red Hat Enterprise Linux (RHEL) 9.5.  Per ulteriori informazioni sul supporto e sulle funzionalità, vederelink:hu-nvme-index.html["Panoramica NVME-oF"^] .

NVMe-oF con RHEL 9.5 non presenta limitazioni note.
...
.Fasi
. Installare RHEL 9.5 sul server.  Una volta completata l'installazione, verificare di avere in esecuzione il kernel RHEL 9.5 richiesto:
+
[source, cli]
----
uname -r
