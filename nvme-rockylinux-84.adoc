...
summary: 'Come configurare l"host NVMe-oF per Rocky Linux 8.4 con ONTAP' 
---
= Configurare Rocky Linux 8.4 con NVMe-oF per l'archiviazione ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Gli host Rocky Linux 8.4 supportano i protocolli NVMe/FC e NVMe/TCP con Asymmetric Namespace Access (ANA). ANA è equivalente al multipathing ALUA (Asymmetric Logical Unit Access) negli ambienti iSCSI e FCP ed è implementato utilizzando la funzionalità multipath NVMe integrata nel kernel.
...
È possibile utilizzare il supporto e le funzionalità seguenti con la configurazione host NVMe-oF per Rocky Linux 8.4. Prima di avviare il processo di configurazione, è inoltre necessario esaminare le limitazioni note.
...
* Limitazioni note:
+
** Per impostazione predefinita, il multipath NVMe nel kernel è disabilitato per gli host Rocky Linux 8.4 NVMe-oF. Pertanto, è necessario attivarlo manualmente.
** Sugli host Rocky Linux 8.4, NVMe/TCP è una funzionalità di anteprima tecnologica a causa di problemi aperti. Per ulteriori informazioni, fare riferimento alla https://access.redhat.com/documentation/en-us/red_hat_enterprise_linux/8/html-single/8.4_release_notes/index#technology-preview_file-systems-and-storage["Note di rilascio di Rocky Linux 8.4"^] .
...
Utilizzare la seguente procedura per convalidare le versioni minime supportate del software Rocky Linux 8.4.

.Fasi
. Installa Rocky Linux 8.4 sul server. Al termine dell'installazione, verifica di utilizzare il kernel Rocky Linux 8.4 specificato:
+
[source, cli]
----
uname -r
...
Non sono noti problemi per la configurazione host NVMe-oF su Rocky Linux 8.4 con versione ONTAP.
