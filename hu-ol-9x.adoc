...
summary: Installare Linux Host Utilities su un host Oracle Linux 9.x per gestire le operazioni del protocollo FCP e iSCSI con LUN ONTAP . 
---
= Configurare Oracle Linux 9.x per FCP e iSCSI con storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Il software Linux Host Utilities fornisce strumenti di gestione e diagnostica per gli host Linux connessi allo storage ONTAP . Quando si installa Linux Host Utilities su un host Oracle Linux 9.x, è possibile utilizzare Host Utilities per gestire le operazioni del protocollo FCP e iSCSI con LUN ONTAP .
...
È possibile utilizzare il multipathing con Oracle Linux 9.x per gestire le LUN ONTAP .
...
Oracle Linux 9.3, 9.2, 9.1 e 9.0 con storage ONTAP presentano il seguente problema noto:
...
| https://mysupport.netapp.com/site/bugs-online/product/HOSTUTILITIES/BURT/1508554["1508554"^] | L'utility SAN LUN con Emulex HBA richiede collegamenti simbolici dai pacchetti di librerie | Quando si esegue il comando Linux Host Utilities CLI - "sanlun fcp show adapter -v" su un host SAN, il comando fallisce e viene visualizzato un messaggio di errore che indica che le dipendenze della libreria richieste per un rilevamento dell'adattatore bus host (HBA) non possono essere individuate: ---- [root@hostname ~]# sanlun fcp show adapter -v Impossibile individuare la libreria /usr/lib64/libHBAAPI.so Assicurarsi che il pacchetto che installa la libreria sia installato e caricato ---- 
|===
