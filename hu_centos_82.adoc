...
summary: Installa Linux Host Utilities su un host CentOS 8.2 per gestire le operazioni del protocollo FCP e iSCSI con LUN ONTAP . 
---
= Configurare CentOS 8.2 per FCP e iSCSI per l'archiviazione ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Il software Linux host Utilities fornisce strumenti di gestione e diagnostica per gli host Linux connessi allo storage ONTAP. Quando si installa Linux Host Utilities su un host CentOS 8.2, è possibile utilizzare Host Utilities per gestire le operazioni del protocollo FCP e iSCSI con LUN ONTAP.
...
È possibile utilizzare il multipathing con CentOS 8.2 per gestire le LUN ONTAP.
...
Non sono noti problemi per CentOS 8.2 per FCP e iSCSI con storage ONTAP.

Per i problemi noti del kernel compatibile con CentOS Red Hat, vedere link:hu_rhel_82.html#known-problems-and-limitations["problemi noti"] per Red Hat Enterprise Linux (RHEL) 8.2.
