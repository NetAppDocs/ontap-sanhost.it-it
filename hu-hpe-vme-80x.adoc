...
summary: Installare Linux Host Utilities su un host HPE VME 8.0.x per gestire le operazioni dei protocolli FCP e iSCSI con LUN ONTAP. 
---
= Configurare HPE VME 8.0.x per FCP e iSCSI con ONTAP storage
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Il software Linux Host Utilities fornisce strumenti di gestione e diagnostica per gli host Linux connessi allo storage ONTAP. Quando si installa Linux Host Utilities su un host HPE VME 8.0.x, è possibile utilizzare Host Utilities per gestire le operazioni dei protocolli FCP e iSCSI con le LUN ONTAP.



[[step-1-install-the-linux-host-utilities]]
== Passaggio 1: installa le Linux Host Utilities
...
È possibile utilizzare il multipathing con HPE VME 8.0.x per gestire le LUN ONTAP.
...
== Passaggio 3: conferma la configurazione iSCSI per il tuo host
...
* Configura gli archivi dati HPE VME sulle LUN ONTAP, quindi configura la macchina virtuale. Consulta la documentazione HPE del fornitore per ulteriori informazioni.
