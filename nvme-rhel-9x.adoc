...
summary: 'Come configurare l"host NVMe-oF per RHEL 9x con ONTAP' 
---
= Configurare RHEL 9x per NVMe-oF con storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Scopri come configurare gli host NVMe over Fabrics (NVMe-oF) per RHEL 9x. Per ulteriori informazioni sul supporto e sulle funzionalità, vedere link:nvme-rhel-supported-features.html["Supporto e funzionalità RHEL ONTAP"].

NVMe-oF con RHEL 9x presenta le seguenti limitazioni note:
...
.Fasi
. Installare RHEL 9x sul server.  Una volta completata l'installazione, verificare di avere in esecuzione il kernel RHEL 9x richiesto:
+
[source, cli]
----
uname -r
...
NOTE: La nuova iopolicy si applica automaticamente ai dispositivi NetApp ONTAP Controller corrispondenti.  Non è necessario alcun riavvio.
...
L'autenticazione in-band sicura è supportata tramite NVMe/TCP tra un host RHEL 9x e un controller ONTAP .
...
cat /etc/nvme/hostnqn
----
. Genera la chiave dhchap per l'host RHEL 9x.
+
