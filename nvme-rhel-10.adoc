...
summary: 'Come configurare l"host NVMe-oF per RHEL 10.0 con ONTAP' 
---
= Configurare RHEL 10.0 per NVMe-oF con storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Scopri come configurare gli host NVMe over Fabrics (NVMe-oF) per RHEL 10.0.  Per ulteriori informazioni sul supporto e sulle funzionalità, vederelink:hu-nvme-index.html["Panoramica NVME-oF"^] .

*NVMe-oF con RHEL 10.0 presenta le seguenti limitazioni note:*
...
.Fasi
. Installare RHEL 10.0 sul server.  Una volta completata l'installazione, verificare di avere in esecuzione il kernel RHEL 10.0 richiesto:
+
[source, cli]
----
uname -r
...
cat /etc/nvme/hostnqn
----
. Generare la chiave dhchap per l'host RHEL 10.0.
+
