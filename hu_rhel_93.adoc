...
summary: Installare le utilità host Linux su un host RHEL 9,3 per gestire le operazioni dei protocolli FCP e iSCSI con i LUN ONTAP. 
---
= Configurare RHEL 9,3 per FCP e iSCSI con storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
È possibile utilizzare multipathing con RHEL 9,3 per gestire i LUN di ONTAP.
...
La versione RHEL 9,3 con storage ONTAP presenta i seguenti problemi noti:
