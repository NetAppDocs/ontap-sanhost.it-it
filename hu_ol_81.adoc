...
summary: Descrive come utilizzare Oracle Linux 8.1 con ONTAP 
---
= Utilizza Oracle Linux 8.1 con ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
È possibile utilizzare le impostazioni di configurazione dell'host SAN di ONTAP per configurare Oracle Linux 8,1 con ONTAP come destinazione.
...
Per Oracle Linux 8.1 il file /etc/multipath.conf deve esistere, ma non è necessario apportare modifiche specifiche al file. Oracle Linux 8.1 è compilato con tutte le impostazioni necessarie per riconoscere e gestire correttamente le LUN ONTAP.
...
Il sistema operativo Oracle Linux 8.1 viene compilato per riconoscere le LUN ONTAP e impostare automaticamente correttamente tutti i parametri di configurazione.
...
NOTE: Per configurare Oracle Linux 8,1 Red Hat Enterprise kernel (RHCK), utilizzare il link:hu_rhel_81.html#recommended-settings["impostazioni consigliate"] per Red Hat Enterprise Linux (RHEL) 8,1.



== Problemi noti

Non ci sono problemi noti per Oracle Linux 8,1 con ONTAP release.


NOTE: Per i problemi noti di Oracle Linux (kernel compatibile con Red Hat), consultare link:hu_rhel_81.html#known-problems-and-limitations["problemi noti"] Per Red Hat Enterprise Linux (RHEL) 8.1.
