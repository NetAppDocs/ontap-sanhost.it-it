...
---
= Configurare RHEL 8.10 per NVMe-oF con storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Scopri come configurare gli host NVMe over Fabrics (NVMe-oF) per RHEL 8.10.  Per ulteriori informazioni sul supporto e sulle funzionalità, vederelink:hu-nvme-index.html["Panoramica NVME-oF"^] .

*NVMe-oF con RHEL 8.10 presenta le seguenti limitazioni note:*

* L'avvio SAN tramite il protocollo NVMe-oF non è attualmente supportato.
* Il multipath NVMe nel kernel è disabilitato per impostazione predefinita sugli host NVMe-oF in RHEL 8.10; è necessario abilitarlo manualmente.
...
.Fasi
. Installare RHEL 8.10 sul server.  Una volta completata l'installazione, verificare di avere in esecuzione il kernel RHEL 8.10 richiesto:
+
[source, cli]
----
uname -r
...
----
. Sull'host RHEL 8.10, controllare `hostnqn` stringa a `/etc/nvme/hostnqn` :
+
[source, cli]
----
