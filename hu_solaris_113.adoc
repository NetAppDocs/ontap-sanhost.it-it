...
** NetApp supporta l'utilizzo delle tecnologie di virtualizzazione Solaris laddove la configurazione complessiva sia supportata da Oracle e qualsiasi partizione con accesso diretto ai LUN sia elencata link:https://imt.netapp.com/matrix/#welcome["IMT"^] in una configurazione supportata. Questo include i container root, i domini I/O LDOM e LDOM che utilizzano NPIV per accedere ai LUN.
