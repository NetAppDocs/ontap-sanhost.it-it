...
Per un elenco aggiornato delle configurazioni supportate, consultare la http://mysupport.netapp.com/matrix3["Tool di matrice di interoperabilità (IMT)"^].
...
.Fasi
. Installare Oracle Linux 8.3 GA sul server. Una volta completata l'installazione, verificare di eseguire il kernel Oracle Linux 8.3 GA specificato. Per l'elenco aggiornato delle versioni supportate, vedere la https://mysupport.netapp.com/matrix/["Tool di matrice di interoperabilità"^] .
+
[listing]
----
# uname -r
...
[NOTE]
====
Se si intende eseguire traffico NVMe e SCSI sullo stesso host Oracle Linux 8,3, NetApp consiglia di utilizzare il multipath NVMe in-kernel per gli namespace ONTAP e per le LUN ONTAP, `dm-multipath` rispettivamente. Ciò significa anche che gli spazi dei nomi ONTAP devono essere inseriti nella blacklist `dm-multipath` per evitare di `dm-multipath` richiedere tali dispositivi dello spazio dei nomi. Questo può essere fatto aggiungendo l'impostazione _enable_Foreign_ al `/etc/multipath.conf` file:
...
.Fasi
. Verificare di utilizzare l'adattatore supportato. Per l'elenco corrente delle schede di rete supportate, vedere https://mysupport.netapp.com/matrix/["Tool di matrice di interoperabilità"^] .
+
[listing]
----
...
Emulex LPe36002-M64 2-Port 64Gb Fibre Channel Adapter
----
. Verificare di utilizzare il firmware Broadcom lpfc e il driver della posta in arrivo consigliati. Per l'elenco aggiornato dei driver della scheda di rete e delle versioni firmware supportate, vedere https://mysupport.netapp.com/matrix/["Tool di matrice di interoperabilità"^] .
+
[listing]
----
