...
summary: Installa Linux Host Utilities su un host Ubuntu 26.04 per gestire le operazioni dei protocolli FCP e iSCSI con le LUN ONTAP. 
---
= Configura Ubuntu 26.04 per FCP e iSCSI con lo storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Il software Linux Host Utilities fornisce strumenti di gestione e diagnostica per gli host Linux connessi allo storage ONTAP. Quando installi Linux Host Utilities su un host Ubuntu 26.04, puoi usare Host Utilities per gestire le operazioni dei protocolli FCP e iSCSI con le LUN ONTAP.
...
Puoi usare il multipathing con Ubuntu 26.04 per gestire le LUN ONTAP.
