...
summary: Installare le utilità host Linux su un host RHEL 8,0 per gestire le operazioni dei protocolli FCP e iSCSI con i LUN ONTAP. 
---
= Configurare RHEL 8,0 per FCP e iSCSI con storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
È possibile utilizzare multipathing con RHEL 8,0 per gestire i LUN di ONTAP.
...
La versione RHEL 8,0 con storage ONTAP presenta i seguenti problemi noti:
