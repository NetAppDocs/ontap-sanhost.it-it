...
Quando installi Linux Host Utilities, non cambia nessuna impostazione o comportamento sul tuo host.
...
NOTE: Linux Host Utilities ha una dipendenza di installazione dalla libreria libHBAAPI.so. Devi installare uno dei pacchetti sopra elencati, anche se un host è destinato a una configurazione solo iSCSI. In alternativa, puoi usare gli strumenti a livello di sistema operativo per la gestione di LUN e percorsi.
