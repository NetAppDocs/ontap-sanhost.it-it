...
NOTE: Per configurare Oracle Linux 8,10 RedHat Enterprise kernel, utilizzare il link:hu_rhel_810.html#recommended-settings["impostazioni consigliate"] per Red Hat Enterprise Linux (RHEL) 8,10.



== Impostazioni KVM

È inoltre possibile utilizzare <<recommended_settings_ol_810,impostazioni consigliate>>per configurare una macchina virtuale basata su kernel (KVM). Non sono necessarie modifiche per configurare un KVM perché la LUN è mappata all'hypervisor.
