...
A partire da Proxmox VE 9.0, la iopolicy predefinita per NVMe-oF è impostata su `queue-depth`. Se si desidera modificare la iopolicy in `round-robin`, anch'essa disponibile come opzione configurabile, modificare il file delle regole udev come segue:
...
----
. Modificare la regola in modo che  `queue-depth` sia cambiata in  `round-robin`, come mostrato nel seguente esempio:
+
[listing]
----
...
----
. Verifica l'attuale iopolicy per il tuo sottosistema. Sostituisci <subsystem> con il nome del tuo sottosistema, ad esempio nvme-subsys0.
+
[source, cli]
----
...
.Mostra l'output di esempio AFF, FAS o ASA
[%collapsible]
=====
[listing, subs="+quotes"]
----
