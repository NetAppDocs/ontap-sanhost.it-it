...
| 1517321 | Gli host Oracle Linux 8,8 NVMe-of creano PDC duplicati | Negli host OL 8,8 NVMe-of, i controller di rilevamento persistente (PDC) vengono creati passando `-p` l'opzione al `nvme discover` comando. Per una determinata combinazione iniziatore-target, ogni esecuzione del `nvme discover` comando dovrebbe creare un PDC. Tuttavia, a partire da OL 8.x, gli host NVMe-of creano PDC duplicati. In questo modo si sprecano risorse sia sull'host che sulla destinazione. | https://bugzilla.oracle.com/bugzilla/show_bug.cgi?id=18118["18118"^] 
|===