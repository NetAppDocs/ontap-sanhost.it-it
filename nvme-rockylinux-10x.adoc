...
summary: 'Come configurare l"host NVMe-oF per Rocky Linux 10.x con storage ONTAP' 
---
= Configurare Rocky Linux 10.x per NVMe-oF con storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
Scopri come configurare gli host NVMe over Fabrics (NVMe-oF) per Rocky Linux 10.x. Per ulteriori informazioni sul supporto e sulle funzionalità, vedere link:nvme-rockylinux-supported-features.html["Supporto e funzionalità di Rocky Linux ONTAP"].

NVMe-oF con Rocky Linux 10.x presenta le seguenti limitazioni note:
...
.Fasi
. Installare Rocky Linux 10.x sul server.  Una volta completata l'installazione, verifica di avere installato il kernel Rocky Linux 10.x richiesto:
+
[source, cli]
----
uname -r
...
L'autenticazione in-band sicura è supportata tramite NVMe/TCP tra un host Rocky Linux 10.x e un controller ONTAP .
...
cat /etc/nvme/hostnqn
----
. Genera la chiave dhchap per l'host Rocky Linux 10.x.
+
