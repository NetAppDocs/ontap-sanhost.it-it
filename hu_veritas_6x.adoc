...
---
= Utilizzo di Veritas Storage Foundation 6 per Linux con NetApp ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
Questo documento fornisce indicazioni sulle impostazioni di configurazione degli host SAN ONTAP per la release della serie 6 di Veritas Storage Foundation per le piattaforme Linux e Oracle (basate su RHCK) con protocolli FC, FCoE e iSCSI.
...
Verificare che la configurazione soddisfi i requisiti di sistema. Per ulteriori informazioni, consultare https://mysupport.netapp.com/matrix/imt.jsp?components=65623;64703;&solution=1&isHWU&src=IMT["Tool di matrice di interoperabilità NetApp"^] E Veritas HCL Matrix.
