...
---
= Utilizzo di Veritas Infoscale 8 per Linux con NetApp ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
Questo documento fornisce indicazioni sulle impostazioni di configurazione degli host SAN ONTAP per la release 8 della serie Veritas Infosscale Storage Foundation per le piattaforme Red Hat Enterprise Linux e Oracle Linux (basate su RHCK) con protocolli FC, FCoE e iSCSI.
