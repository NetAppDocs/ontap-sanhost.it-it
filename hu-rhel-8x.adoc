...
summary: Installare Linux Host Utilities su un host RHEL 8.x per gestire le operazioni del protocollo FCP e iSCSI con LUN ONTAP . 
---
= Configurare RHEL 8.x per FCP e iSCSI con storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Il software Linux Host Utilities fornisce strumenti di gestione e diagnostica per gli host Linux connessi allo storage ONTAP . Quando si installano le Linux Host Utilities su un host Red Hat Enterprise Linux (RHEL) 8.x, è possibile utilizzare le Host Utilities per gestire le operazioni del protocollo FCP e iSCSI con LUN ONTAP .
...
È possibile utilizzare il multipathing con RHEL 8.x per gestire le LUN ONTAP .
...
RHEL 8.x con storage ONTAP presenta i seguenti problemi noti.
