...
[role="lead"]
Prima di eseguire la risoluzione di eventuali errori NVMe-of per gli host RHEL, OL e SLES, verificare che sia in esecuzione una configurazione conforme alle specifiche dello strumento matrice di interoperabilità (IMT), quindi procedere con i passaggi successivi per eseguire il debug di eventuali problemi lato host.
