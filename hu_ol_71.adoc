...
summary: Installare Linux Host Utilities su un host Oracle Linux 7.1 per gestire le operazioni del protocollo FCP e iSCSI con LUN ONTAP. 
---
= Configurare Oracle Linux 7.1 per FCP e iSCSI con storage ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Il software Linux host Utilities fornisce strumenti di gestione e diagnostica per gli host Linux connessi allo storage ONTAP. Quando si installa Linux Host Utilities su un host Oracle Linux 7.1, è possibile utilizzare Host Utilities per gestire le operazioni del protocollo FCP e iSCSI con LUN ONTAP.
...
Puoi utilizzare multipathing con Oracle Linux 7,1 per gestire i LUN di ONTAP. Oracle Linux 7.1 supporta Unbreakable Enterprise kernel (UEK) R3 e UEK R4. Il sistema operativo si avvia con il kernel UEK R3 per impostazione predefinita.


NOTE: Puoi usare il link:hu_rhel_71.html#rhel-rhck["impostazioni consigliate per Red Hat Enterprise Linux (RHEL) 7.1"] per configurare il kernel compatibile con Red Hat per Oracle Linux 7.1.
