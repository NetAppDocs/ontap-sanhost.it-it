...
---
= Utilizzo di Red Hat Enterprise Linux 8.2 con NetApp ONTAP
:toc: macro
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
...
Per Red Hat Enterprise Linux (RHEL) 8.2 il file /etc/multipath.conf deve esistere, ma non è necessario apportare modifiche specifiche al file. RHEL 8.2 è compilato con tutte le impostazioni necessarie per riconoscere e gestire correttamente le LUN ONTAP.
...
Non ci sono problemi noti per RHEL 8.2.
