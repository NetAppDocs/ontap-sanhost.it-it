...
summary: 'Come configurare l"host NVMe-oF per Rocky Linux 9.1 con ONTAP' 
---
= Configurare Rocky Linux 9.1 con NVMe-oF per l'archiviazione ONTAP
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
[role="lead"]
Gli host Rocky Linux 9.1 supportano i protocolli NVMe/FC e NVMe/TCP con Asymmetric Namespace Access (ANA). ANA è equivalente al multipathing ALUA (Asymmetric Logical Unit Access) negli ambienti iSCSI e FCP ed è implementato utilizzando la funzionalità multipath NVMe integrata nel kernel.
...
È possibile utilizzare il supporto e le funzionalità seguenti con la configurazione host NVMe-oF per Rocky Linux 9.1. Prima di avviare il processo di configurazione, è inoltre necessario esaminare le limitazioni note.
...
+
** Rocky Linux 9.1 abilita per impostazione predefinita il multipath NVMe nel kernel per gli spazi dei nomi NVMe, eliminando la necessità di impostazioni esplicite.
...
Utilizzare la seguente procedura per convalidare le versioni minime supportate del software Rocky Linux 9.1.

.Fasi
. Installa Rocky Linux 9.1 sul server. Al termine dell'installazione, verifica di utilizzare il kernel Rocky Linux 9.1 specificato:
+
[source, cli]
----
uname -r
...
La configurazione host NVMe-oF per Rocky Linux 9.1 con storage ONTAP presenta il seguente problema noto:
...
| 1503468 | `nvme list-subsys` il comando restituisce i controller NVMe ripetuti elencati per un dato sottosistema | IL  `nvme list-subsys` Il comando dovrebbe restituire un elenco univoco di controller NVMe associati a un determinato sottosistema. In Rocky Linux 9.1, il  `nvme list-subsys` Il comando restituisce i controller NVMe con il rispettivo stato ANA per tutti gli spazi dei nomi appartenenti a un determinato sottosistema. Tuttavia, lo stato ANA è un attributo per ogni spazio dei nomi, pertanto, se si elenca la sintassi del comando del sottosistema per un determinato spazio dei nomi, è necessario visualizzare voci univoche del controller NVMe con lo stato del percorso. 
|===